...
const Login = () => {
  ...
  const validateResponse = (response, errored) => {
    ...
    if (response.status !==201 && !response.data.message) {
      ...
      enqueueSnackbar('Something went wrong. Check that the backend is running, reachable and returns valid JSON.',{variant:"warning" ,autoHideDuration:5000});
      return false;
    }
     ...
     if (!response.data.success  && response.data.message) {
      ...
      enqueueSnackbar(response.data.message,{variant:"warning" ,autoHideDuration:5000});
        return false;
    ...
    >
      ...
      <Box className="content">
        <Stack spacing={2} className="form">
          ...
          />
         {formData.loading ? <Box display="flex" flexDirection="row" justifyContent="center">  <CircularProgress size={25} /> </Box> : <Button className="button" variant="contained" onClick={()=>login(formData)}>
            LOGIN TO QKART
          </Button>}
